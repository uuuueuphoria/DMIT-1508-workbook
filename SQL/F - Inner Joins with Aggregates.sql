...
SELECT  P.PositionDescription,                    --  <-- non-aggregate
...
--3. How many payments where made for each payment type. Display the PaymentTypeDescription and the count.
 ...
 SELECT		P.PaymentTypeDescription,
			COUNT (T.PaymentID) AS 'Number of payment'
 FROM		PaymentType AS P
	INNER JOIN Payment AS T ON P.PaymentTypeID=T.PaymentTypeID
...
--6. What is the highest, lowest and average payment amount for each payment type Description?
 ...
 SELECT		T.PaymentTypeDescription,
			MAX(P.Amount) AS 'Highest',
			MIN(P.Amount) AS 'Lowest',
			AVG(P.Amount) AS 'average'
 FROM		PaymentType AS T
	INNER JOIN Payment AS P ON T.PaymentTypeID=P.PaymentTypeID
 GROUP BY	T.PaymentTypeDescription
