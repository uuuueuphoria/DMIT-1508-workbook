...
SELECT  PaymentTypeID,                              -- Non-aggregate column (btw, it's a FK)
        COUNT(PaymentID) AS 'Count of Pay Type' -- Aggregate column
FROM    Payment
GROUP BY PaymentTypeID
--let us see what the payment types are
...
FROM    Payment
GROUP BY PaymentTypeID
ORDER BY COUNT(PaymentTypeID) DESC --DESCENDING ORDER
...
SELECT		AVG(C.CourseCost) AS 'Average Course Cost',
			C.CourseHours
FROM		Course AS C
...
SELECT		R.StaffID,
			AVG(R.Mark) AS 'Average course mark'
...
SELECT		AVG(S.BalanceOwing) AS 'AVG balance',
			S.Gender
FROM		Student AS S
GROUP BY	S.Gender
